...
def hitungHargaDenganHotel():
    if kelas_input.get() == 'Ekonomi':
        hargaTiketTotal = int(int(hargaMaskapaiPilihan[0]) * 0.9) * int(input_jumlah.get())
    elif kelas_input.get() == 'Bisnis':
        hargaTiketTotal = (int(int(hargaMaskapaiPilihan[0]) * 0.9)) * 2 * int(input_jumlah.get())
    else:
        hargaTiketTotal = (int(int(hargaMaskapaiPilihan[0]) * 0.9)) * 3 * int(input_jumlah.get())
    e.append(hargaTiketTotal)


def hitungHargaTanpaHotel():
    if kelas_input.get() == 'Ekonomi':
        hargaTiketTotal1 = int(hargaMaskapaiPilihan[0]) * int(input_jumlah.get())
    elif kelas_input.get() == 'Bisnis':
        hargaTiketTotal1 = (int(hargaMaskapaiPilihan[0])) * 2 * int(input_jumlah.get())
    else:
        hargaTiketTotal1 = (int(hargaMaskapaiPilihan[0])) * 3 * int(input_jumlah.get())
    f.append(hargaTiketTotal1)


def tiketBill():
    perhitunganHarga()
    hitungHargaDenganHotel()
    ...
    Label(bayar_frame, text='Tiket Maskapai Total = Rp' + str(e[0])).place(x=0, y=295)
    Label(bayar_frame, text='Penginapan = Rp' + str(hargaHotelPilihan[0])).place(x=0, y=320)
    Label(bayar_frame, text='TOTAL : Rp' + str(int(hargaHotelPilihan[0]) + int(e[0]))).place(x=0, y=345)


def tiketBillLewati():
    perhitunganHarga()
    hitungHargaTanpaHotel()
    Label(lewati_frame, text='TICKET BILL').place(x=220, y=160)
    Label(lewati_frame, text='Tagihan yang perlu Anda bayar :').place(x=0, y=195)
    ...
    Label(lewati_frame, text='Tiket Maskapai Total = Rp' + str(f[0])).place(x=0, y=295)
    Label(lewati_frame, text='TOTAL : Rp' + str(f[0])).place(x=0, y=320)
...
myFrame = [main_page, main_frame, input_frame, issa_frame, bayar_frame, berhasil_frame, tiket_frame, lewati_frame]
...
tombol3 = Button(issa_frame, text='Lewati', width=10, command=lambda: frame_1(lewati_frame)).place(x=130, y=200)
...
Label(lewati_frame, text='Bayar').place(x=0, y=80)
...
tombol11 = Button(lewati_frame, text='Bayar', width=10, command=lambda: frame_1(bayar_frame)).place(x=50, y=110)
tombol_bayar1 = Button(lewati_frame, text='Input metode', command=lambda: tiketBillLewati())
tombol_bayar1.place(x=150, y=110)
